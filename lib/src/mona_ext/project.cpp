...
/*
 * This file is mostly a copy-paste of MONA/DFA/project.c, except
 * the acceptance condition in dfaProject.
 */
...
unsigned proj_term1(unsigned state1, unsigned state2) {
  ...
  if ((res = (int)(uintptr_t)lookup_in_hash_tab(htbl_set, (long)s, 0))) {
    ...
    res = make_sset(size, s, SET_BDD_NOT_CALCULATED, state1,
                    state2); /* optimize if equal? */
    return (res);
  }
}
...
bdd_ptr proj_term2(unsigned set_index1, unsigned set_index2) {
  ...
  *e3 = -1; /* Terminate the new set */
  ...
  if ((res = (int)(uintptr_t)lookup_in_hash_tab(htbl_set, (long)s, 0))) {
    ...
    res =
        make_sset((e3 - s), s, SET_BDD_NOT_CALCULATED, set_index1, set_index2);
    return (res);
  }
}
...
unsigned eval_bdd(int ss) {
  ...
  if (ssets[ss].sq == SET_BDD_NOT_CALCULATED) {
    ...
    (void)bdd_apply2_hashed(bddm_res, bdd_roots(bddm_res)[root1], bddm_res,
                            bdd_roots(bddm_res)[root2], bddm_res, &proj_term2);
...
DFA* dfaUniversalProject(DFA* a, unsigned var_index) {
  ...
  for (i = 0; i < a->ns; i++) { /* Update bdd's */
    ...
    ssets[i].sq = bdd_roots_length(bddm_res) -
                  1; /* bdd_roots_length(bddm_res) - 1 == i */
  ...
  ssets[a->s].permanent = next_state++; /* Should be 0 */
  {
    ...
    while (lh) {
      ...
      /* Insert leaves */
      (void)bdd_apply1(bddm_res, bdd_roots(bddm_res)[root_place], bddm_res_,
                       &proj_term3);
      /*evaluate bdd_roots(bddm_res) at each iteration since bdd_apply1 is
       * called*/
      lh = lh->next;
    }
