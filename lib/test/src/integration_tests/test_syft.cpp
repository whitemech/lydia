...
static void _dataset_test(const std::filesystem::path &dataset_path) {
  ...
  for (const auto &datapath : it) {
    ...
    auto expected_dfa = load_mona_dfa(mona_file);
    auto actual_dfa = to_dfa_from_formula_file(lydia_file, *strategy);
    ...
    //  commented because there are differences in how the empty trace is
    //  handled.
    //    REQUIRE(expected_dfa->get_nb_states() >= actual_dfa->get_nb_states());
