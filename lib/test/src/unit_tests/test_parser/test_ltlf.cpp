...
//      REQUIRE(*parsedAnd == *actualAnd_false_true);
//    }
//    SECTION("test parsing true & ff & tt") {
...
//    auto tt = context->makeLtlfTrue();
//    auto ff = context->makeLtlfFalse();
//    auto actualOr_true_false = context->makeLtlfOr(set_ltlf_formulas({tt,
//    ff})); auto actualOr_false_true =
//    context->makeLtlfOr(set_ltlf_formulas({ff, tt}));
//
//    SECTION("test parsing true || ff") {
...
//
//    auto actualOr_true_false = context->makeLtlfOr(set_ltlf_formulas({tt,
//    ff})); auto actualNot_or = context->makeLtlfNot(actualOr_true_false);
//
...
//  }
//
//  TEST_CASE("LTLfDriver LTLfTemporal", "[parser][ltlf]") {
//    auto context = std::make_shared<AstManager>();
//    auto driver = LTLfDriver(context);
//
...
//    auto ptr_test_re = context->makeTestRegex(context->makeLtlfFalse());
//
//    auto ptr_tt = context->makeLtlfTrue();
//    auto ptr_ff = context->makeLtlfFalse();
//    auto ptr_end = context->makeLtlfBox(ptr_prop_re_true, ptr_ff);
//    auto ptr_last = context->makeLtlfDiamond(ptr_prop_re_true, ptr_end);
//
//    auto actualDiamond_prop_re_tt =
//        context->makeLtlfDiamond(ptr_prop_re_a, ptr_tt);
//    auto actualDiamond_seq_re_tt = context->makeLtlfDiamond(ptr_seq_re,
//    ptr_tt); auto actualDiamond_union_re_tt =
//        context->makeLtlfDiamond(ptr_union_re, ptr_tt);
//    auto actualDiamond_star_re_tt = context->makeLtlfDiamond(ptr_star_re,
//    ptr_tt); auto actualDiamond_test_re_tt =
//    context->makeLtlfDiamond(ptr_test_re, ptr_tt);
//
//    auto actualBox_prop_re_tt = context->makeLtlfBox(ptr_prop_re_a, ptr_tt);
//    auto actualBox_seq_re_tt = context->makeLtlfBox(ptr_seq_re, ptr_tt);
//    auto actualBox_union_re_tt = context->makeLtlfBox(ptr_union_re, ptr_tt);
//    auto actualBox_star_re_tt = context->makeLtlfBox(ptr_star_re, ptr_tt);
//    auto actualBox_test_re_tt = context->makeLtlfBox(ptr_test_re, ptr_tt);
//
//    auto actualDiamond_a_end = context->makeLtlfDiamond(ptr_prop_re_a,
//    ptr_end); auto actualDiamond_a_last =
//    context->makeLtlfDiamond(ptr_prop_re_a, ptr_last); auto actualBox_a_end =
//    context->makeLtlfBox(ptr_prop_re_a, ptr_end); auto actualBox_a_last =
//    context->makeLtlfBox(ptr_prop_re_a, ptr_last);
//
...
//    auto context = std::make_shared<AstManager>();
//    auto driver = LTLfDriver(context);
//    auto a_tt = context->makeLtlfDiamond(
//        context->makePropRegex(context->makePropAtom("a")),
//        context->makeLtlfTrue());
//    auto b_tt = context->makeLtlfDiamond(
...
//      auto expected_f =
//          context->makeLtlfOr(set_ltlf_formulas{context->makeLtlfNot(a_tt),
//          b_tt});
...
//        context->makePropRegex(context->makePropAtom("b")),
//        context->makeLtlfTrue());
//    auto not_a_tt = context->makeLtlfNot(a_tt);
//    auto not_b_tt = context->makeLtlfNot(b_tt);
//
//    SECTION("test parsing (<a>tt <-> <b>tt)") {
//      std::istringstream f("<a>tt <-> <b>tt");
//      driver.parse(f);
//      auto actual_f = driver.result;
//      auto left_to_right = context->makeLtlfOr(set_ltlf_formulas{not_a_tt,
//      b_tt}); auto right_to_left =
//      context->makeLtlfOr(set_ltlf_formulas{not_b_tt, a_tt}); auto expected_f
//      =
//          context->makeLtlfAnd(set_ltlf_formulas{left_to_right,
//          right_to_left});
//      REQUIRE(*actual_f == *expected_f);
//    }
//    SECTION("test parsing (<a>tt <-> <a>tt)") {
...
//      auto actual_f = driver.result;
//      auto left_to_right = context->makeLtlfOr(set_ltlf_formulas{not_a_tt,
//      a_tt}); auto right_to_left =
//      context->makeLtlfOr(set_ltlf_formulas{not_a_tt, a_tt}); auto expected_f
//      =
//          context->makeLtlfAnd(set_ltlf_formulas{left_to_right,
//          right_to_left});
//      REQUIRE(*actual_f == *expected_f);
